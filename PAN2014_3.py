    ...
    while i<len(plags):
        ...
        if cont_ol==0:
            ...
            for j in range(1,cont_ol+1):
                if flag==0:
                    sents_i=list(range(plags[i][1][0],plags[i][1][1]+1))
                    range_i=list(range(plags[i][0][0],plags[i][0][1]+1))
                else:
                    sents_i=list(range(res[-1][1][0],res[-1][1][1]+1))
                    range_i=list(range(res[-1][0][0],res[-1][0][1]+1))
                sents_j=list(range(plags[i+j][1][0],plags[i+j][1][1]+1))
                sim_i_ol=0.0
                sim_j_ol=0.0
                sim_i_nol=0.0
                sim_j_nol=0.0
                ...
                for sent in sents_i:
                    ...
                    if sent in sents_j:
                        ...
                        sim_i_nol+=sim_max
                        cont_i_nol_sents+=1
                range_j=list(range(plags[i+j][0][0],plags[i+j][0][1]+1))
                for sent in sents_j:
                    sim_max=0.0
                    for k in range_j:
                        sim=cosine_measure(susp_bow[sent],src_bow[k])
...
class SGSPLAG:
    def __init__(self, susp, src, outdir):
        ...
        self.susp_size = 1
        self.min_sentlen = 2         
        self.min_match_words = 5     
        self.min_plaglen = 80     

        #  “+20 的 summary variant”
        self.rssent = 'no'
        self.tf_idf_p = 'yes'
        self.rem_sw = 'no'
    ...
    def compare(self):
        ...
        for c in range(len(self.susp_bow)):
            for r in range(len(self.src_bow)):
                if sum(self.susp_bow[c].values()) < self.min_match_words or \
                    sum(self.src_bow[r].values()) < self.min_match_words:
                    continue
